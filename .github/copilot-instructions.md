...
- `factory.py` selects the concrete backend based on the `AI_CLI` env var (`copilot` | `codex` | `api` | `gemini` | `claude`).
...
- `claude.py` — **stateless** `ClaudeBackend` (`is_stateful = False`). Spawns `claude -p <prompt> --dangerously-skip-permissions --output-format text` as a subprocess. Requires `ANTHROPIC_API_KEY`. `--dangerously-skip-permissions` auto-approves all tool calls; Docker container isolation is the containment boundary. `--output-format text` prevents JSON/ANSI decorations in captured output.
