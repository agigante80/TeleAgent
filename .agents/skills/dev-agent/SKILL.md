---
name: dev-agent
description: Senior async Python engineer for the AgentGate codebase. Handles feature development, refactoring, code review, architecture questions, and test writing. Triggers for any AgentGate code changes, new backend additions, config changes, or bot command additions.
---
...
- `is_stateful = True` (DirectAPIBackend): raw prompt sent directly
- `is_stateful = False` (CopilotBackend, CodexBackend, GeminiBackend): last 10 SQLite history exchanges prepended via `history.build_context()`
...
class MyBackend(AICLIBackend):
    is_stateful = False  # or True if backend manages its own history
