...
> Last updated: 2026-03-16 (roadmap-sync: removed 6 implemented features — api-key-scheme, docs-align-sync, gemini-cli-backend, here-broadcast-command, request-cancellation, thinking-persist)
...
| 2.5 | `/gate schedule` — recurring shell commands or AI prompts | [→ features/schedule.cells.md](features/schedule.md) |
...
| 2.18 | ✅ `AI_PROVIDER` explicit validation — require `AI_PROVIDER` to be non-empty when `AI_CLI=api` | [→ features/ai-provider-validation.md](features/ai-provider-validation.md) |
